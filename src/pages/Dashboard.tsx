...
import { Package, Clock, CheckCircle2, AlertCircle, MapPin, Camera, User } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-6">
      <div className="flex justify-between items-end mb-12">
        <div>
          <h1 className="text-xs uppercase tracking-[0.3em] font-bold text-stone-400 mb-2">{role} Panel</h1>
          <h2 className="text-5xl font-bold tracking-tighter italic serif">Welcome, {user?.displayName?.split(' ')[0]}.</h2>
        </div>
...
const BookingCard: React.FC<{ booking: any; role: string }> = ({ booking, role }) => {
      ...
      <div className="flex flex-col md:flex-row justify-between gap-6">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs text-stone-400 font-medium">
              {booking.createdAt?.toDate ? format(booking.createdAt.toDate(), 'MMM d, yyyy') : 'Recently'}
            </span>
          </div>
          <h4 className="text-xl font-bold mb-2 uppercase tracking-tight">{booking.serviceType.replace(/_/g, ' ')}</h4>
        ...
        <div className="md:w-48 flex flex-col justify-between items-end">
          <div className="text-right">
            ...
            <p className="text-[10px] uppercase tracking-widest font-bold text-stone-400">{booking.bookingStatus.replace(/_/g, ' ')}</p>
          </div>
